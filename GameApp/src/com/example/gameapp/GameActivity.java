...
public class GameActivity extends Activity {
	...
	final private SoundPool gameSound = new SoundPool(1, AudioManager.STREAM_MUSIC, 0); //sound pool that plays sound effects
	...
	private boolean soundState; //gets assigned soundstate intent
	private int goodbuttonsound; //id of good button
	private int badbuttonsound; //id of good button
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (action == MotionEvent.ACTION_DOWN) {
				...
				//if the player touches the screen, it's neither the round display time or the waiting before it,
							...
							if (soundState){ gameSound.play(goodbuttonsound, 1.0f, 1.0f, 0, 0, 1.0f); }
						...
						if (newWin == TOUCH_RESET) {
							if (soundState){ gameSound.play(badbuttonsound, 1.0f, 1.0f, 0, 0, 1.0f); }
	...
	public void startMedia() {
		...
		//load sound files into the SoundPool and keep their ids
		badbuttonsound = gameSound.load(this, R.raw.bad_button, 1);
		goodbuttonsound = gameSound.load(this, R.raw.good_button, 1);
	}
}